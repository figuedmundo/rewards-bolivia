...
describe('RedeemPointsUseCase', () => {
  ...
  describe('execute', () => {
    it('should throw an error if customer is not found', async () => {
      ...
      await expect(useCase.execute({ businessId: '1', pointsToRedeem: 100, ticketTotal: 500 }, '1')).rejects.toThrow(
    ...
    it('should throw an error if customer has insufficient balance', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({ id: '1', pointsBalance: 50 });
      await expect(useCase.execute({ businessId: '1', pointsToRedeem: 100, ticketTotal: 500 }, '1')).rejects.toThrow(
        new HttpException('Insufficient points balance', HttpStatus.BAD_REQUEST),
      );
    });

    it('should throw an error if business is not found', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({ id: '1', pointsBalance: 200 });
    ...
    it('should throw an error if redemption value exceeds 30% of ticket total', async () => {
      mockPrismaService.user.findUnique.mockResolvedValue({ id: '1', pointsBalance: 4000 });
      mockPrismaService.business.findUnique.mockResolvedValue({ id: '1', name: 'Test Business' });

      await expect(useCase.execute({ businessId: '1', pointsToRedeem: 3001, ticketTotal: 100 }, '1')).rejects.toThrow(
        new HttpException('Redemption value exceeds 30% of ticket total', HttpStatus.BAD_REQUEST),
      );
    });
    ...
    it('should create a transaction and return the result', async () => {
      ...
      mockPrismaService.user.findUnique.mockResolvedValueOnce(customer).mockResolvedValueOnce(updatedCustomer);
      ...
      const result = await useCase.execute({ businessId: '1', pointsToRedeem: 100, ticketTotal: 400 }, '1');
